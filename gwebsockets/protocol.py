# Copyright 2014 Open Source, Daniel Narvaez, Alex Guzman
#
...
def parse_frame(buf):
    ...
    if has_mask:
        ...
        payload = [chr(b ^ mask[i % 4]) for i, b in enumerate(payload)]
...
def parse_message(buf):
    ...
    if opcode == OPCODE_CLOSE:
        if len(payload) >= 2:
            ...
            close_code = struct.unpack('!H', str.encode(close_message)[:2])[0]
...
def _make_frame(message, opcode):
    ...
    if msg_length < 126:
        header += struct.pack('!B',msg_length)
    elif msg_length < (1 << 16):
        header += struct.pack('!B',126) + struct.pack('!H', msg_length)
    else:
        header += struct.pack('!B',127) + struct.pack('!Q', msg_length)
